...
class Spaceship {
    ...
    func moveLeft() {
        position -= 1
        print("The spaceship moved left. New position: \(position)")
    }
    func moveRight() {
        position += 1
        print("The spaceship moved right. New position: \(position)")
    }
    func wasHit() {
        health -= 5
        if health <= 0 {
            print("Sorry, your ship was hit one too many time. Do you want to play again?")
        } else {
            print("The spaceship was hit! Current health: \(health)")
        }
    }
}
